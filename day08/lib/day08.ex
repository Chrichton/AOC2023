defmodule Day08 do
  ...
  def solve(input) do
    ...
    |> then(fn {directions, left_rights} ->
      traverse(directions, left_rights, "AAA", &(&1 == "ZZZ"))
    end)
  end

  def traverse(directions, left_rights, from, to_func) do
    Enum.reduce_while(directions, {from, 1}, fn direction, {from, steps} ->
      destination = destination(from, direction, left_rights)

      if to_func.(destination),
        do: {:halt, steps},
  ...
  def solve2(input) do
    ...
    |> read_input()
    |> then(fn {directions, left_rights} ->
      Enum.filter(left_rights, fn {left, _right} -> String.ends_with?(left, "A") end)
      |> Enum.map(fn {left, _right} ->
        traverse(directions, left_rights, left, &String.ends_with?(&1, "Z"))
      end)
      |> lcm()
    end)
  end

  def lcm([number]), do: number

  def lcm([number | numbers]) do
    number
    |> ElixirMath.lcm(lcm(numbers))
  end

  ########## Too Slow for second star
